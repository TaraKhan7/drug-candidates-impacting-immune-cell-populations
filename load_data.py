...
table_creation = ["""CREATE TABLE IF NOT EXISTS Projects (
...
subjects_df = df[["subject", "condition","age","sex","project", "sample_type"]].drop_duplicates()
...
subjects_final_df = subjects_merged[["subject", "condition", "age", "sex", "project_id"]]
# Insert to database
...
samples_df = df[["sample", "time_from_treatment_start","b_cell","cd8_t_cell","cd4_t_cell","nk_cell","monocyte", "subject"]].drop_duplicates()
samples_merged = samples_df.merge(subjects_db, on=["subject"]) # merge
samples_final_df = samples_merged[["sample", "time_from_treatment_start","b_cell","cd8_t_cell","cd4_t_cell","nk_cell","monocyte", "subject_id"]]
# Insert to database
